...
class ThemeService {
  ...
  // 设置主题 by ID (for the new theme color picker)
  ...
  // 还原主题（用于取消操作）：直接 apply 指定 ID 并还原 mode
  ...
  private applyByAppearance(appearance: 'light' | 'dark'): void {
    ...
    const savedId = localStorage.getItem('gearl-theme-id');
    if (savedId) {
