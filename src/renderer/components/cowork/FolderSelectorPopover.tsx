...
const PathTooltip: React.FC<PathTooltipProps> = ({ path, anchorRect, visible }) => {
  ...
  return (
    <div
      style={style}
      className="px-3.5 py-2.5 text-[13px] leading-relaxed rounded-xl shadow-xl dark:bg-background bg-background text-foreground border-border border break-all pointer-events-none"
    >
      {path}
    </div>
  );
...
const FolderSelectorPopover: React.FC<FolderSelectorPopoverProps> = ({
  ...
  return (
    <>
      ...
      <div
        ref={popoverRef}
        className="absolute bottom-full left-0 mb-2 w-56 rounded-lg border border-border dark:bg-surface bg-surface shadow-lg z-50"
      >
        {/* Add Folder option */}
        <button
      ...
      {showRecentSubmenu && (
        <div
          ref={submenuRef}
          className="fixed w-64 max-h-80 overflow-y-auto rounded-lg border border-border dark:bg-surface bg-surface shadow-lg z-[60]"
