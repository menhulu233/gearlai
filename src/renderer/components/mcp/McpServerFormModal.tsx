...
const McpServerFormModal: React.FC<McpServerFormModalProps> = ({
  ...
  const inputClass = 'w-full px-3 py-2 text-sm rounded-xl dark:bg-background bg-background dark:text-foreground text-foreground dark:placeholder-secondary placeholder-secondary border dark:border-border border-border focus:outline-none focus:ring-2 focus:ring-primary';
  const readOnlyInputClass = inputClass + ' opacity-60 cursor-not-allowed';
  const labelClass = 'text-xs font-semibold tracking-wide dark:text-secondary text-secondary';
  const kvInputClass = 'flex-1 px-2 py-1.5 text-sm rounded-lg dark:bg-background bg-background dark:text-foreground text-foreground border dark:border-border border-border focus:outline-none focus:ring-1 focus:ring-primary';
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg mx-4 rounded-2xl dark:bg-surface bg-surface border-border shadow-2xl p-6 max-h-[80vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-5">
          <div className="text-lg font-semibold dark:text-foreground text-foreground">
            {modalTitle}
          </div>
        </div>
        ...
        <div className="space-y-4">
                  ...
                  <div key={index} className="flex items-center gap-2">
                    ...
                    {!row.required && (
                      <button
                        ...
                        className="p-1 text-secondary dark:text-secondary hover:text-red-500 dark:hover:text-red-400 transition-colors flex-shrink-0"
                      >
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-3 py-1.5 text-xs rounded-lg border-border dark:text-secondary text-secondary hover:bg-surface-raised transition-colors"
            >
