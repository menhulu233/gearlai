      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1">
          <button
            ...
            className={`flex items-center justify-between w-full rounded-lg dark:bg-surface bg-surface border-border border focus:border-primary focus:ring-1 focus:ring-primary/40 text-foreground px-4 py-2.5 text-sm ${className}`}
          ...
          {isOpen && (
            <div className="absolute z-10 w-full mt-1 overflow-auto rounded-md popover-enter shadow-popover max-h-60 focus:outline-none">
              <ul
                className="py-1 overflow-auto text-sm dark:bg-surface bg-surface border border-border rounded-lg"
                role="listbox"
                aria-labelledby={id}
              >
                {options.map((option) => (
