...
const WindowTitleBar: React.FC<WindowTitleBarProps> = ({
  ...
  const containerClassName = inline
    ...
    : `window-controls-floating non-draggable absolute top-0 right-0 z-[55] flex h-full items-center gap-0.5 rounded-bl-xl pl-1 pb-1 pt-0.5 transition-colors ${
      ...
      isOverlayActive
        ? 'bg-transparent'
        : 'dark:bg-surface/35 bg-surface/35 backdrop-blur-sm'
    } ${className}`.trim();
  ...
  return (
    ...
    >
      <button
        ...
        className="non-draggable h-8 w-8 inline-flex items-center justify-center rounded-lg transition-colors text-secondary hover:bg-surface-raised"
        aria-label="Minimize"
        title="Minimize"
      >
