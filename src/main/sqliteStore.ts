...
export class SqliteStore {
  ...
  private initializeTables(basePath: string) {
    ...
    try {
      const stCols = this.db.pragma('table_info(scheduled_tasks)') as Array<{ name: string }>;
      const stColNames = stCols.map((c) => c.name);

      if (!stColNames.includes('expires_at')) {
        this.db.exec('ALTER TABLE scheduled_tasks ADD COLUMN expires_at TEXT');
      }

      if (!stColNames.includes('notify_platforms_json')) {
        this.db.exec("ALTER TABLE scheduled_tasks ADD COLUMN notify_platforms_json TEXT NOT NULL DEFAULT '[]'");
      }
    } catch {
  ...
  // Expose save method for external use (e.g., CoworkStore) - no-op for better-sqlite3
