...
const resolveInlineAttachmentDir = (cwd?: string): string => {
  ...
  return path.join(app.getPath('temp'), 'gearlai', 'attachments');
};
...
const buildLogExportFileName = (): string => {
  ...
  return `gearlai-logs-${datePart}-${timePart}.zip`;
};
...
const disableGpu =
  process.env.GEARLAI_DISABLE_GPU === '1' ||
  process.env.GEARLAI_DISABLE_GPU === 'true' ||
...
if (!gotTheLock) {
  ...
  ipcMain.handle('mcp:fetchMarketplace', async () => {
    const url = app.isPackaged
      ? 'https://api-overmind.youdao.com/openapi/get/luna/hardware/gearlai/prod/mcp-marketplace'
      : 'https://api-overmind.youdao.com/openapi/get/luna/hardware/gearlai/test/mcp-marketplace';
    try {
  ...
  const initApp = async () => {
    ...
    const defaultProjectDir = path.join(os.homedir(), 'gearlai', 'project');
